...
#import "ARITweak.h"
#import "../UI/ARISplashViewController.h"
...
- (void)toggleEditView:(BOOL)toggle withTargetLocation:(NSString *)targetLoc {
    if(toggle) {
        ...
        view.currentSettingLabel.text = @"请选择：";
        // Update our label
...
- (void)askForEdit {
    ...
    UIAlertAction *root = [UIAlertAction actionWithTitle:@"Dock栏"
                                                   style:UIAlertActionStyleDefault
                                                 handler:^(UIAlertAction *action) {
                                                     [self toggleEditView:1
                                                         withTargetLocation:@"dock"];
                                                 }];
    UIAlertAction *dock = [UIAlertAction actionWithTitle:@"桌面布局"
                                                   style:UIAlertActionStyleDefault
                                                 handler:^(UIAlertAction *action) {
                                                     [self toggleEditView:1
    ...
    if([manager boolValueForKey:@"showBackground"]) {
        UIAlertAction *background = [UIAlertAction actionWithTitle:@"桌面背景"
                                                             style:UIAlertActionStyleDefault
                                                           handler:^(UIAlertAction *action) {
                                                               [self toggleEditView:1
    ...
    if([manager boolValueForKey:@"showWelcome"]) {
        UIAlertAction *welcome = [UIAlertAction actionWithTitle:@"欢迎文字"
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction *action) {
                                                            [self toggleEditView:1
