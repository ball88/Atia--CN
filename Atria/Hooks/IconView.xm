...
- (NSArray *)applicationShortcutItems {
	...
	if(kIconIsInRoot(self)) {
		[items addObject:[self _atriaGenerateItemWithTitle:@"编辑桌面布局" type:@"me.ren7995.atria.edit.hs"]];
		if([[ARITweak sharedInstance] boolValueForKey:@"showWelcome"]) {
			[items addObject:[self _atriaGenerateItemWithTitle:@"编辑欢迎文字" type:@"me.ren7995.atria.edit.welcome"]];
		}

		if([[ARITweak sharedInstance] boolValueForKey:@"showBackground"]) {
			[items addObject:[self _atriaGenerateItemWithTitle:@"编辑模糊背景" type:@"me.ren7995.atria.edit.background"]];
		}
	} else if(kIconIsInDock(self)) {
		[items addObject:[self _atriaGenerateItemWithTitle:@"编辑Dock栏" type:@"me.ren7995.atria.edit.dock"]];
	}
